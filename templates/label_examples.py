...
Reads examples/<NAME>.png and writes examples/labeled/<NAME>.png with a navy caption band
above the page carrying the template's NAME.

NO LETTERS. They were A-F until 09/10 and they were actively misleading: the picker orders
the menu by what the report sounds like, so the letter and the position disagreed - "C" could
appear first. A label that has to be cross-referenced against a shifting list is worse than
no label. The name is stable and needs no key.
...
TEMPLATES = [
    ("JTI_Record_Summary", "Record Summary"),
    ("JTI_ESeries_Summary", "eSeries Screen"),
    ("JTI_Tabular_List", "List"),
    ("JTI_Grouped_Summary", "Grouped Summary"),
    ("JTI_Statement", "Statement"),
    ("JTI_Wide_Table", "Wide Table (landscape)"),
]
...
def label(stem, name):
    ...
    d.text((PAD + 8, BAND / 2 - (nb[3] - nb[1]) / 2 - nb[1]), name, font=nf, fill=WHITE)
